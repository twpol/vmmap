...
process_memory::process_memory(const process& process) : _process(process)
{
	...
	if (NULL == hProcess) {
		std::tcerr << std::dec << _process.process_id() << ": OpenProcess failed: " << std::hex << std::setw(8) << std::setfill(_T('0')) 
	...
	HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32		
		| TH32CS_SNAPTHREAD | TH32CS_SNAPHEAPLIST, _process.process_id());
	if (INVALID_HANDLE_VALUE != hSnapshot) {
		...
		if (Module32First(hSnapshot, &module)) {
			...
			std::tcerr << std::dec << _process.process_id() << ": Module32First failed: " << std::hex << std::setw(8) << std::setfill(_T('0')) 
				<< GetLastError() << std::endl;
		}

		/*
		**
		this procedure, as if using the Heap32First and Heap32Next functions, 
		does not work if the process to be analyzed is 32 bit and you are using the 64 bit version of vmmap
		**
		*/
		PDEBUG_BUFFER db = RtlCreateQueryDebugBuffer(0, FALSE);
		RtlQueryProcessDebugInformation(_process.process_id(), PDI_HEAPS | PDI_HEAP_BLOCKS, db);

#ifdef _WIN64
		DWORD64 heapNodeCount = db->HeapInformation ? *PDWORD64(db->HeapInformation) : 0;		
		PDEBUG_HEAP_INFORMATION heapInfo = PDEBUG_HEAP_INFORMATION(PDWORD64(db->HeapInformation) + 1);	
#elif _WIN32
		DWORD heapNodeCount = db->HeapInformation ? *PULONG(db->HeapInformation) : 0;		
		PDEBUG_HEAP_INFORMATION heapInfo = PDEBUG_HEAP_INFORMATION(PULONG(db->HeapInformation) + 1);		
#endif
		for (unsigned long heap_index = 0; heap_index < heapNodeCount; heap_index++) {
			process_heap heap = process_heap(heap_index,
				heapInfo[heap_index].Base, heapInfo[heap_index].Flags & HF32_DEFAULT);//TODO the default flag in this new struct is different
			
			HeapBlock hb = { 0,0,0,0 };
			memset(&hb, 0, sizeof(hb));

			if (GetFirstHeapBlock(&heapInfo[heap_index], &hb)){
				do {
					heap.add_block((ULONG_PTR)hb.dwAddress, (ULONG_PTR)hb.dwAddress + hb.dwSize);
				} while (GetNextHeapBlock(&heapInfo[heap_index], &hb));
			}
		...
		//
		//		//Debug start
		//		//if (heap.th32HeapID == 12517376) {
		//			HEAPENTRY32 heap_block = { sizeof(HEAPENTRY32) };
		//			if (Heap32First(&heap_block, heap.th32ProcessID, heap.th32HeapID)) {
		//				do {
		//						//<< std::setfill(_T(' ')) << (heap_block.dwFlags & LF32_FIXED ? "FIXED" : "     ")
		//						//<< " " << (heap_block.dwFlags & LF32_FREE ? "FREE" : "    ") << " "
		//						//<< (heap_block.dwFlags & LF32_MOVEABLE ? "MOVEABLE" : "        ")
		//						//<< " lock=" << heap_block.dwLockCount << ", resvd=" << heap_block.dwResvd << ", handle=" << heap_block.hHandle; 
		//					if (1) {
		//						std::tcout << "   BLOCK(" << std::hex << std::setfill(_T('0')) << std::setw(16) << heap_block.dwAddress
		//							<< "-" << std::setw(16) << (heap_block.dwAddress + heap_block.dwBlockSize) << ") ";
		...
		//						}
		//						else {
		//							std::tcerr << std::dec << _process.process_id() << ": ReadProcessMemory failed: " << buffer_length
		//								<< " bytes, " << std::hex << std::setw(8) << std::setfill(_T('0')) << GetLastError() << std::endl;
		//							std::tcerr << std::hex << std::setfill(_T('0')) << "start address: " << (ULONG_PTR)heap_block.dwAddress
		//								<< " end address: " << (ULONG_PTR)heap_block.dwAddress + heap_block.dwBlockSize << std::endl;
		...
		//			}
		//			else {
		//				std::tcerr << std::dec << _process.process_id() << ": Heap32First failed: " << std::hex << std::setw(8) << std::setfill(_T('0'))
		//					<< GetLastError() << std::endl;
		//			}
		//		//}//Debug end
		//	} while (Heap32ListNext(hSnapshot, &heap));
		//} else {
		//	std::tcerr << std::dec << _process.process_id() << ": Heap32ListFirst failed: " << std::hex << std::setw(8) << std::setfill(_T('0')) 
		//		<< GetLastError() << std::endl;
		//}
				...
				if (thread.th32OwnerProcessID == _process.process_id()) {
					...
					if (NULL != hThread) {
						...
						if (GetThreadContext(hThread, &context_64bit)) {
							...
							std::tcerr << std::dec << _process.process_id() << ": GetThreadContext(64bit) failed: " << std::hex << std::setw(8) 
								<< std::setfill(_T('0')) << GetLastError() << std::endl;
						}
						...
						if (GetThreadContext(hThread, &context_32bit)) {
							...
							std::tcerr << std::dec << _process.process_id() << ": GetThreadContext(32bit) failed: " << std::hex << std::setw(8) 
								<< std::setfill(_T('0')) << GetLastError() << std::endl;
						}
						if (GetCurrentThreadId() != thread.th32ThreadID) {
						...
						std::tcerr << std::dec << _process.process_id() << ": OpenThread failed: " << std::hex << std::setw(8) << std::setfill(_T('0')) 
							<< GetLastError() << std::endl;
					}
				}
			} while (Thread32Next(hSnapshot, &thread));
		} else {
			std::tcerr << std::dec << _process.process_id() << ": Thread32First failed: " << std::dec << std::setw(8) << std::setfill(_T('0')) 
				<< GetLastError() << std::endl;
		}

		CloseHandle(hSnapshot);
	} else {
		std::tcerr << std::dec << _process.process_id() << ": CreateToolhelp32Snapshot failed: " << std::hex << std::setw(8) << std::setfill(_T('0'))
			<< GetLastError() << std::endl;
	}
	...
	// Collect virtual memory allocations...
	{
		...
		for (unsigned long long address = 0; address < maxAddress; address += info.RegionSize) {
			...
			if (info_size != sizeof(info)) {
				std::tcerr << std::dec << _process.process_id() << "VirtualQueryEx returned unexpected info_size (" << std::hex << info_size << ", expected " 
					<< std::hex << sizeof(info) << ")" << std::endl;
			}
			if (info.BaseAddress != (void*)address) {
				std::tcerr << std::dec << _process.process_id() << "VirtualQueryEx returned unexpected BaseAddress (" << std::hex << info.BaseAddress 
					<< ", expected " << std::hex << (void*)address << ")" << std::endl;
			}
			...
			if (info.State == MEM_FREE && systemInfo.dwAllocationGranularity > systemInfo.dwPageSize) {
				unsigned long long unusable_end = ((address + systemInfo.dwAllocationGranularity - 1) / systemInfo.dwAllocationGranularity) 
	...
	if (info->State == MEM_FREE) {
		...
		if ((_type == PMGT_SHAREABLE) || (_type == PMGT_PRIVATE)) {
			...
			for (std::list<process_heap>::const_iterator heap = memory.heaps().begin(); heap != memory.heaps().end(); heap++) {
				if (((unsigned long long)info->BaseAddress <= heap->base()) && 
					((unsigned long long)info->BaseAddress + (unsigned long long)info->RegionSize >= heap->base())) {
...
process_memory_block::process_memory_block(const process_memory& memory, 
	const HANDLE hProcess, process_memory_group& group, const PMEMORY_BASIC_INFORMATION info) : _protection(PMP_NONE)
{
	...
	if ((_type != PMBT_FREE) && (group.type() == PMGT_PRIVATE)) {
		...
		for (std::list<process_stack>::const_iterator stack = memory.stacks().begin(); stack != memory.stacks().end(); stack++) {
			if (((unsigned long long)info->BaseAddress <= stack->base()) 
				&& ((unsigned long long)info->BaseAddress + (unsigned long long)info->RegionSize >= stack->base())) 
			{
